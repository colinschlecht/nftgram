...
describe("nftgramio contract", function () {
  ...
  it("allows you to find the token ID", async function () {
    const arr = []
    await contract.mintNFT(accounts[0], tokenURI);
    await contract.mintNFT(accounts[1], tokenURI);
    await contract.mintNFT(accounts[1], tokenURI);
    await contract.mintNFT(accounts[0], tokenURI);
    await contract.mintNFT(accounts[1], tokenURI);
    const supply = await contract.totalSupply();
    ...
    for(let i = 0; i < total; i++){
      try{
        const id = await contract.ownerOf(i)
        if(id === accounts[0]){
          arr.push(i)
        }
      } catch(err){
    ...
    console.log(await contract.ownerOf(parseInt(await contract.tokenByIndex(arr[arr.length -1]))))
    console.log(parseInt(await contract.tokenByIndex(arr[arr.length -1])))
    console.log(await contract.ownerOf(arr[arr.length -1]))
